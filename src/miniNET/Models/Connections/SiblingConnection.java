...
public class SiblingConnection implements ConnectionManipulator {
	...
	@Override
	public void remove() {
		sibling.getConnections().get(RelationshipConstant.SIBLING).remove(person);
		if (sibling.getConnections().get(RelationshipConstant.SIBLING).isEmpty()) {
			sibling.getConnections().remove(RelationshipConstant.SIBLING);
		}
	}
